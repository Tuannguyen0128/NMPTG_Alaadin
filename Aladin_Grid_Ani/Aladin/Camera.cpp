...
void CCamera::Update(DWORD dt, int const &AladinX, int const &AladinY, int const &mapWidth, int const &mapHeight)
{
	//Truyền thống=> đôi lúc sẽ giật
	//x = AladinX - CAMERA_WIDTH / 2
	//y = AladinY+CAMERA_HEIGHT/2;

	//chỉnh cam mượt hơn xíu bằng xét khoảng
	if (AladinX>=x+CAMERA_WIDTH / 2+10)
	{
		x = AladinX - CAMERA_WIDTH / 2-10;
	}
	else if(AladinX<=x + CAMERA_WIDTH / 2 - 20)
	{
		x = AladinX - CAMERA_WIDTH / 2 + 20;
	}
	if (x < 0)
	{
		x = 0;
